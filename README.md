<div align="center">

   # Hey there, I am Subhin MS. Welcome to my Github..👋 
   <br/>
  
</div>

## 👨🏻‍💻 About Me

💡 For now i am trying to master **Web Development.** <br/>
🔭 I'm building a **dating application** now. <br/>
👯 I’m looking to collaborate on projects to build **Full-Stack Applications.** <br/>
🌱 Also i'm on track for learning more about **Artificial Intelligence and Systems Design.** <br/>
✍️ In my free time, i pursue **automation and robotics.** <br/>
💬 Feel free to reach out to me for creating **websites or webapplications.** <br/>
✉️ You can shoot me an email at **subhinsubramanian@gmail.com** <br/>
📄 Please have a look at my portfolio **https://portfolio-subhi.netlify.app/** to see my works, education and skills. <br/>

<br/>

## 🛠 My Tech Stack


| CATEGORY | TECHNOLOGIES |
|--------|--------------|
| **Frontend** | Next.js, React, TypeScript, HTML |
| **State Management** | Redux, Zustand, TanStack Query |
| **Backend** | Node.js, Express.js |
| **Styling** | CSS, Tailwind CSS, DaisyUI, Flowbite |
| **Database** | MongoDB, Mongoose |
| **Authentication** | JWT, Google OAuth |
| **Media & File Handling** | Cloudinary, Multer |
| **Realtime / Chat Services** | GetStream |
| **Payment Integrations** | Razorpay |
| **Deployment & Hosting** | Vercel, Netlify, Render |
| **Languages** | JavaScript, C, C++ (Beginner) |
| **Cloud / BaaS** | Firebase |
| **Tools & Others** | Git, GitHub, npm, VS Code, Insomnia, Postman |

<br/>

## 💼 My Projects

- Next.js Chat Application 
- MERN Web Application for a Real Estate Agency 
- MERN Web Application for a Blogging community 
- MERN E-commerce Application for computer hardware enterprise 
- A simple Wwebsite for a Digital Media Marketing Agency

## 📫 Connect with me 

📸 Instagram: https://www.instagram.com/deve.lopermonkey/ <br/>
▶️ YouTube: https://www.youtube.com/@Developer_Monkey-x3z <br/>
💼 LinkedIn: https://www.linkedin.com/in/subhin-ms-433a54167/ <br/>
🌐 Portfolio: https://portfolio-subhi.netlify.app/ <br/>



